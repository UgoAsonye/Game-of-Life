...
// =============================================================================
	...
	// WRITE ME
        int myrow = getRow();
        int mycolumn = getColumn();
        int counter = 0;
        //rows
        for (int r = myrow - 1; r <= myrow + 1; r++){
            //columns
            for (int c = mycolumn - 1; c <= mycolumn + 1; c++){
                if (r == myrow && c == mycolumn || r >= _grid.getRows() || c >= _grid.getColumns() || r < 0 || c < 0) {}
                else {
     ...
     **/
    public void evolve () {
        //will become dead rules
        if (countLiveNeighbors() < 2 || countLiveNeighbors() >= 4){
            _willBeAlive = false;
        }
        //will stay alive rule
        if (_isAlive == true && (countLiveNeighbors() == 3 || countLiveNeighbors() == 2)) {
            _willBeAlive = true;
        }
        //dead comes to life
        if (_isAlive == false && countLiveNeighbors() == 3){
            _willBeAlive = true;
        }
	// WRITE ME
